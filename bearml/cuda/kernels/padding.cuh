...
namespace bearml {
    namespace cuda {
        ...
        template<typename T>
        void launch_padd_with_constant(const T* d_in, T* d_out, int batch_size, int in_rows, int in_cols, int out_rows, int out_cols, int padding, int r_start, int r_end, int c_start, int c_end, T constant, cudaStream_t stream = nullptr);
        ...
        #define INSTANTIATE_PADDING(T) \
            ...
            template void launch_padd_with_constant<T>(const T* d_in, T* d_out, int batch_size, int in_rows, int in_cols, int out_rows, int out_cols, int padding, int r_start, int r_end, int c_start, int c_end, T constant, cudaStream_t stream);
        #endif
