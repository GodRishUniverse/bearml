...
namespace bearml {
    namespace cuda {
        ...
        #define INSTANTIATE_GEMM(T) \
            ...
            template __global__ void gemm_kernel_contiguous<T>(int batchsize, const int M, const int K, const int N, T alpha, T beta, int64_t row_aware_strde_a, int64_t col_aware_stride_a, int64_t row_aware_strde_b, int64_t col_aware_stride_b, const T* __restrict__ a, const T* __restrict__ b, T* c); \
            ...
            template void launch_gemm_contiguous<T>(const T* d_a, const T* d_b, T* d_c, int batchsize, int m, int k, int n, T alpha, T beta, int64_t row_aware_strde_a, int64_t col_aware_stride_a, int64_t row_aware_strde_b, int64_t col_aware_stride_b, cudaStream_t stream);
        #endif
