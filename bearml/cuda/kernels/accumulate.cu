        ...
        void launch_accumulate_kernel(T *d_data, T *d_out, std::vector<int> data_shape, std::vector<int> out_shape, int64_t size, int64_t offset_new_shape, int64_t offset_old, reductions::ReductionOps op,  bool keepdims, cudaStream_t stream) {
            ...
            switch (op) {
                case reductions::ReductionOps::SUM:
                    bearml::cuda::launch_sum_acc_kernel<T>(d_data, d_out, offset_new_shape, offset_old, size, stream);
                    break;
                ...
                case reductions::ReductionOps::MEAN: {
                    ...
                    bearml::cuda::launch_elementwise_contiguous_with_constant<T>(d_out, element_count, d_out, out_shape, OP_Code::OP_DIV, LHS_RHS_Code::OP_RHS, stream);
                    break;
                };
                // TODO: test this below
                case reductions::ReductionOps::PROD:
                    bearml::cuda::launch_prod_acc_kernel<T>(d_data, d_out, offset_new_shape, offset_old, size, stream);
                    break;
