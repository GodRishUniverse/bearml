...
namespace bearml{
   ...
   class Node {
        ...
        inline static bearml::reductions::ReductionOps reduction_op = bearml::reductions::ReductionOps::SUM;
    public:
        T val;
        ...
        Node(T value ) : val(value) , grad([&value]() {
                if constexpr (std::is_same<T, double>::value) {
                    ...
                    return T(value.getShape(), value.getDevice()  );
                }
            }()) {
        }
        ...
        static void set_reduction(bearml::reductions::ReductionOps op) {
            reduction_op = op;
        }

        // a utility to get the current reduction operation
        static bearml::reductions::ReductionOps get_reduction() {
            return reduction_op;
        }
        ...
        friend std::shared_ptr<Node<T>> elementwise_binary_node_operators(std::shared_ptr<Node<T>> a, std::shared_ptr<Node<T>> b, OP_Code op){
            ...
            switch(op) {
                ...
                case OP_Code::OP_MAX:
                    if constexpr (bearml::is_tensor_v<T>) {
                        ...
                        node  = make_node(T::max(a->val, b->val));
                    } else {
                ...
                case OP_Code::OP_MIN:
                    if constexpr (bearml::is_tensor_v<T>) {
                        ...
                        node  = make_node(T::min(a->val, b->val));
                    } else {
                ...
                case OP_Code::OP_HADAMARD:
                    if constexpr (bearml::is_tensor_v<T>) {
                        ...
                        node  = make_node(bearml::linear_algebra::hadamard(a->val, b->val));
                    } else {
                        node = make_node(a->val * b->val);
                    }
                    break;
            ...
            node->backward_fn = [weak_a, weak_b, weak_node, op](){
                ...
                switch (op) {
                    ...
                    case OP_Code::OP_MUL:
                        if constexpr (bearml::is_tensor_v<T>){
                            // operator* now consumes col-major transposed views natively (case 6 / 2D)
                            // and batchedMatMul densifies internally for 3D+, so the explicit .contiguous() is gone.
                    ...
                    case OP_Code::OP_DIV:
                        ...
                        // dc/db = grad * (-a/b^2)
                        if constexpr (bearml::is_tensor_v<T>) {
                            accumulate_grad(a_locked->grad, bearml::linear_algebra::hadamard(node_locked->grad, 1.0/b_locked->val), a_locked->grad);
                            accumulate_grad(b_locked->grad, bearml::linear_algebra::hadamard(node_locked->grad, -1.0 * a_locked->val/ bearml::linear_algebra::hadamard(b_locked->val, b_locked->val)), b_locked->grad);
                        } else {
                    ...
                    case OP_Code::OP_MAX:
                        if constexpr (bearml::is_tensor_v<T>) {
                            T a_mask = bearml::linear_algebra::mask_of_greater_than_equal_to(a_locked->val,b_locked->val);
                            T b_mask = bearml::linear_algebra::mask_of_greater_than_equal_to(b_locked->val,a_locked->val);

                            accumulate_grad(a_locked->grad,bearml::linear_algebra::hadamard(node_locked->grad, a_mask), a_locked->val);
                            accumulate_grad(b_locked->grad,bearml::linear_algebra::hadamard(node_locked->grad, b_mask), b_locked->val);
                        } else {
                    ...
                    case OP_Code::OP_MIN:
                        if constexpr (bearml::is_tensor_v<T>) {
                            T a_mask = bearml::linear_algebra::mask_of_less_than_equal_to(a_locked->val,b_locked->val);
                            T b_mask = bearml::linear_algebra::mask_of_less_than_equal_to(b_locked->val,a_locked->val);
                    ...
                    case OP_Code::OP_HADAMARD:
                        ...
                        if constexpr (bearml::is_tensor_v<T>) {
                            accumulate_grad(a_locked->grad, bearml::linear_algebra::hadamard(node_locked->grad, b_locked->val), a_locked->val);
                            accumulate_grad(b_locked->grad, bearml::linear_algebra::hadamard(node_locked->grad, a_locked->val), b_locked->val);
                        } else {
        ...
        friend std::shared_ptr<Node<T>> operator+(double scalar, std::shared_ptr<Node<T>> b){
            T scalar_tensor({1}, b->val.getDevice());
        ...
        friend std::shared_ptr<Node<T>> operator*(double scalar, std::shared_ptr<Node<T>> b){
                ...
                node->backward_fn = [scalar, weak_b, weak_node](){
                    ...
                    if constexpr (bearml::is_tensor_v<T>){
                        ...
                        b_locked->grad += bearml::linear_algebra::reduce(scalar * node_locked->grad, temp_b, reduction_op); // grad_b = scalar * grad
                    }else{
                       // TODO
                    }
                };
        ...
        friend std::shared_ptr<Node<T>> operator/(double divisor, std::shared_ptr<Node<T>> a){
                ...
                node->backward_fn = [weak_a, weak_node,  divisor]() {
                    ...
                    if constexpr (bearml::is_tensor_v<T>){
                        a_locked->grad += bearml::linear_algebra::hadamard(node_locked->grad,
                            -divisor / bearml::linear_algebra::hadamard(a_locked->val, a_locked->val));
                    } else {
        ...
        friend std::shared_ptr<Node<T>> elementwise_unary_node_operators(std::shared_ptr<Node<T>> a, OP_Code op, int pad_amount = 0, Padding_Op_Code padding_mode = Padding_Op_Code::PAD_CONSTANT, double constant_value = 0){
            ...
            switch(op) {
                case OP_Code::OP_EXP:
                    if constexpr (bearml::is_tensor_v<T>)   node =  make_node(T::exp(a->val));
                    else node = make_node(exp(a->val));
                    break;
                case OP_Code::OP_SIN:
                    if constexpr (bearml::is_tensor_v<T>)   node =  make_node(T::sin(a->val));
                    else node = make_node(sin(a->val));
                    break;
                case OP_Code::OP_COS:
                    if constexpr (bearml::is_tensor_v<T>)   node =  make_node(T::cos(a->val));
                    else node = make_node(cos(a->val));
                    break;
                case OP_Code::OP_TAN:
                    if constexpr (bearml::is_tensor_v<T>)   node =  make_node(T::tan(a->val));
                    else node = make_node(tan(a->val));
                    break;
                case OP_Code::OP_SINH:
                    if constexpr (bearml::is_tensor_v<T>)   node =  make_node(T::sinh(a->val));
                    else node = make_node(sinh(a->val));
                    break;
                case OP_Code::OP_COSH:
                    if constexpr (bearml::is_tensor_v<T>)   node =  make_node(T::cosh(a->val));
                    else node = make_node(cosh(a->val));
                    break;
                case OP_Code::OP_TANH:
                    if constexpr (bearml::is_tensor_v<T>)   node =  make_node(T::tanh(a->val));
                    else node = make_node(tanh(a->val));
                    break;
                case OP_Code::OP_TRANSPOSE:
                    if constexpr (bearml::is_tensor_v<T>)   node =  make_node(a->val.transpose());
                    else node = make_node(a->val); // same as input for non-Tensor types
                    break;
                case OP_Code::OP_ABS:
                    if constexpr (bearml::is_tensor_v<T>)   node =  make_node(T::abs(a->val));
                    else node = make_node(abs(a->val));
                    break;
                case OP_Code::OP_LOG:
                    if constexpr (bearml::is_tensor_v<T>)  node  = make_node(T::log(a->val));
                    else node = make_node(std::log(a->val));
                    break;
                case OP_Code::OP_MEAN_FOR_GRAD:
                    if constexpr (bearml::is_tensor_v<T>)  node  = make_node(T::mean(a->val));
                    else node = make_node(a->val);
                    break;
                case OP_Code::OP_SQRT:
                    if constexpr (bearml::is_tensor_v<T>)  node  = make_node(T::sqrt(a->val));
                    else node = make_node(std::sqrt(a->val));
                    break;

                // TODO - implement this below(need slicing operation)
                case OP_Code::OP_PAD:
                    if constexpr (bearml::is_tensor_v<T>)  node  = make_node(bearml::neural_network::padding(a->val, pad_amount, padding_mode, static_cast<bearml::tensor_element_t<T>>(constant_value)));
            ...
            node->backward_fn = [weak_a, weak_node, op, pad_amount, padding_mode, constant_value]() {
                ...
                switch(op) {
                    case OP_Code::OP_EXP:
                        if constexpr (bearml::is_tensor_v<T>) {
                            accumulate_grad(a_locked->grad, bearml::linear_algebra::hadamard(node_locked->grad, node_locked->val), a_locked->grad);  //  dc/da = exp(a)
                        } else {
                    ...
                    case OP_Code::OP_SIN:
                        if constexpr (bearml::is_tensor_v<T>) {
                            accumulate_grad(a_locked->grad, bearml::linear_algebra::hadamard(node_locked->grad, T::cos(a_locked->val)), a_locked->grad);  //  dc/da = cos(a)
                        } else {
                    ...
                    case OP_Code::OP_COS:
                        if constexpr (bearml::is_tensor_v<T>) {
                            accumulate_grad(a_locked->grad, bearml::linear_algebra::hadamard(node_locked->grad, (-1.0 *  T::sin(a_locked->val))), a_locked->grad);  //  dc/da = -sin(a)
                        } else {
                    ...
                    case OP_Code::OP_TAN:
                        if constexpr (bearml::is_tensor_v<T>) {
                            auto tan_val = T::tan(a_locked->val);
                            //  dc/da = sec^2(a) = 1 + tan^2(a)
                            accumulate_grad(a_locked->grad, bearml::linear_algebra::hadamard(node_locked->grad, bearml::linear_algebra::hadamard(node_locked->grad, (1.0 + bearml::linear_algebra::hadamard(tan_val, tan_val)))), a_locked->grad);
                        } else {
                    ...
                    case OP_Code::OP_SINH:
                        if constexpr (bearml::is_tensor_v<T>) {
                            accumulate_grad(a_locked->grad, bearml::linear_algebra::hadamard(node_locked->grad, T::cosh(a_locked->val)), a_locked->grad); //  dc/da = cosh(a)
                        } else {
                    ...
                    case OP_Code::OP_COSH:
                        if constexpr (bearml::is_tensor_v<T>) {
                            accumulate_grad(a_locked->grad, bearml::linear_algebra::hadamard(node_locked->grad, T::sinh(a_locked->val)), a_locked->grad); //  dc/da = sinh(a)
                        } else {
                    ...
                    case OP_Code::OP_TANH:
                        if constexpr (bearml::is_tensor_v<T>) {
                            accumulate_grad(a_locked->grad, bearml::linear_algebra::hadamard(node_locked->grad, (1.0 - bearml::linear_algebra::hadamard(node_locked->val, node_locked->val))), a_locked->grad); //  dc/da = 1 - tanh^2(a)
                        } else {
                    ...
                    case OP_Code::OP_ABS:
                        if constexpr (bearml::is_tensor_v<T>) {
                            T sign_mat = bearml::linear_algebra::sign(a_locked->val);
                            accumulate_grad(a_locked->grad, bearml::linear_algebra::hadamard(node_locked->grad, sign_mat), a_locked->val);
                        } else {
                    ...
                    case OP_Code::OP_LOG:
                        // d/da log(a) = 1/a
                        if constexpr (bearml::is_tensor_v<T>) {
                            accumulate_grad(a_locked->grad, bearml::linear_algebra::hadamard(node_locked->grad, 1.0/a_locked->val), a_locked->val);
                        } else {
                    ...
                    case OP_Code::OP_MEAN_FOR_GRAD:
                        ...
                        if constexpr (bearml::is_tensor_v<T>) {
                            ...
                            T grad_broadcast(a_locked->val.getShape(), a_locked->val.getDevice());
                            T grad_cpu = node_locked->grad.to(bearml::Device::cpu());
                    ...
                    case OP_Code::OP_SQRT:
                        ...
                        // dc/da = 1/(2*sqrt(a))
                        if constexpr (bearml::is_tensor_v<T>) {
                            accumulate_grad(a_locked->grad, bearml::linear_algebra::hadamard(node_locked->grad, 1.0 / (2.0 * T::sqrt(a_locked->val))), a_locked->val);
                        }else{
                            a_locked->grad += node_locked->grad / (2.0 * std::sqrt(a_locked->val));
                        }
                        break;
                    // Padding operation is only supported for tensor inputs
                    case OP_Code::OP_PAD:
                        if constexpr (bearml::is_tensor_v<T>) {
        ...
        static std::shared_ptr<Node<T>> softmax(std::shared_ptr<Node<T>> a, int dim){
            if constexpr (!bearml::is_tensor_v<T>) {
                ...
                node->backward_fn = [weak_node, weak_a, dim_to_use]() {
                    ...
                    T gs = bearml::linear_algebra::hadamard(g, s);
                    T row_sum = gs.accumulate(dim_to_use, bearml::reductions::ReductionOps::SUM, true);
                    T grad_input = bearml::linear_algebra::hadamard(s, g - row_sum);
    ...
    namespace autogradient {
        template <typename T>
        T backward(std::shared_ptr<bearml::Node<T>> end_node, bool accumulate = false);
    }
