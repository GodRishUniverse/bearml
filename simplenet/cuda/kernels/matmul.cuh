...
namespace simplenet {
    namespace cuda {
        ...
        #define INSTANTIATE_GEMM(T) \
            ...
            template __global__ void gemm_kernel_contiguous<T>(int batchsize, const int M, const int K, const int N, T alpha, T beta, T* __restrict__ a, T* __restrict__ b, T* c); \
            ...
            template void launch_gemm_contiguous<T>(T* d_a, T* d_b, T* d_c, int batchsize, int m, int k, int n, T alpha, T beta, cudaStream_t stream);
        #endif
